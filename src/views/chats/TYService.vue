...
import tyStep from '@/assets/tyStep.png'
import ty from '@/assets/ty.png'
import { useStore } from 'vuex'
import { Message } from '@arco-design/web-vue'
import { requestHandle, sSEHandle } from '@/util/tyRequestHandle'
...
//聊天信息
const chats = ref(store.state.tyChats)
  ...
  <a-layout class="vh-100 d-flex overflow-hidden">
    <a-layout-header class="p-2 px-3 border-bottom">
      <div class="d-flex justify-content-between align-items-center">
        通义千问
        <a-space>
          <a-tooltip content="清除聊天信息,开启新会话；但是不会删除记录（官网可见）">
          ...
          <a-space>
            <a-select v-model="chats.model">
              <template v-if="settings.form.requestModel == 'api'">
                <a-option value="qwen-turbo" label="qwen-turbo" />
                <a-option value="qwen-plus" label="qwen-plus" />
                <a-option value="qwen-max" label="qwen-max" />
                <a-option value="qwen-max-longcontext" label="qwen-max-longcontext" />
              </template>
              <template v-else>
                <a-option value="qwen" label="无需配置" />
              </template>
            </a-select>
    ...
    <a-form label-align="left" auto-label-width :model="settings.form"
      ...
      <a-form-item label="api Token" field="apiToken"
        ...
        <a-row>
          ...
          <a-col>
            <a-link>访问获取：https://dashscope.console.aliyun.com/apiKey</a-link>
          </a-col>
        </a-row>
      </a-form-item>
      ...
      <a-form-item label="获取方式">
        <a-image :src="tyStep" width="100"></a-image>
      </a-form-item>
